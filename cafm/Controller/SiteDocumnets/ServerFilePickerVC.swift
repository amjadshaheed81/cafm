//
//  ServerFilePickerVC.swift
//  cafm
//
//  Created by ShitaRam on 13/10/24.
//
...
class ServerFilePickerVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, SpreadsheetViewDataSource, SpreadsheetViewDelegate, UISearchBarDelegate, UITextFieldDelegate {
    ...
    var headerRow: [String] = ["Folder", "Actions"]
...
//    ]
    ...
    weak var delegate : SelectFileVCDelegate?
    ...
    func spreadsheetView(_ spreadsheetView: SpreadsheetView, heightForRow row: Int) -> CGFloat {
        ...
        case .loading, .failed, .noResponse, .noInternet:
            if folderCollection.count == 1 {
                ...
                let headerHeight = getMaxLabelSize(textArray: textArray, font: font, maxWidth: screenWidth - 90, minHeight: minHeight, heightAddition: heightAddition).height
                return headerHeight
            }else {
                if row == 0 || row == 2 {
                    let refSize = CGSize(width: 100, height: 40)
                    ...
                    let headerHeight = getMaxLabelSize(textArray: stringsArray, font: font, maxWidth: screenWidth - 90, minHeight: minHeight, heightAddition: heightAddition).height
                    return headerHeight
                }
            }
        default:
...
//                cell.btnAction.showsMenuAsPrimaryAction = true
                cell.imgView.image = UIImage(systemName: "arrowshape.turn.up.backward")
...
//                }
                if indexPath.row == 1 {
                    ...
                    cell.imgView.image = UIImage(systemName: "eye")
                    ...
                    cell.btnAction.addAction { [weak self] in
                        DispatchQueue.main.async { [weak self] in
                            ...
                            self.delegate?.selectFile(file: self.subFolderDetails?.document?.files?[convertRow])
                            self.dismiss(animated: true)
                        }
                    }
                    return cell
...
protocol SelectFileVCDelegate: AnyObject {
