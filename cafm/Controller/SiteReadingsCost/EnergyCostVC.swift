...
//  cafm
//
//  Created by ShitaRam on 10/10/24.
//
...
class EnergyCostVC: UIViewController, SpreadsheetViewDataSource, SpreadsheetViewDelegate {
    
    @IBOutlet weak var spreadView: SpreadsheetView!
    
    var headerRow: [String] = ["Budget Category", "From Date", "To Date", "Cost (GBP)", "Action"]
    ...
    @IBAction func btbAddClick(_ sender: UIButton) {
        let vc = siteReadingsCostSB.instantiateViewController(withIdentifier: "AddEnergyCostVC") as! AddEnergyCostVC
        vc.item = self.item
        vc.homeVC = self
    ...
    func spreadsheetView(_ spreadsheetView: SpreadsheetView, widthForColumn column: Int) -> CGFloat {
        ...
        if column == 0 || column == 1 || column == 2 || column == 3 {
            ...
            if column == 0 {
                stringsArray = itemArray.compactMap{$0.budgetCategory}
                stringsArray.append(headerRow[column])
            }else if column == 1 {
                stringsArray = itemArray.compactMap{convertDateString($0.fromDate)}
                stringsArray.append(headerRow[column])
            }else if column == 2 {
                stringsArray = itemArray.compactMap{convertDateString($0.toDate)}
                stringsArray.append(headerRow[column])
            }else if column == 3 {
                stringsArray = itemArray.compactMap{String(format: "£%.2f", $0.cost ?? 0.0)}
                stringsArray.append(headerRow[column])
            }
            let maxColumnWidth = getMaxLabelSize(textArray: stringsArray, font: UIFont(name: .MontserratSemiBold, size: textFontSize), minWidth: 50, widthAddition: 12+12, maxWidth: 200).width
            return maxColumnWidth
        ...
        default:
            if row == 0 {
                ...
                optionArray.append(contentsOf: [
                    itemArray[row-1].budgetCategory,
                    convertDateString(itemArray[row-1].fromDate),
                    convertDateString(itemArray[row-1].toDate),
                    String(format: "£%.2f", itemArray[row-1].cost ?? 0.0)
                ])
    ...
    func spreadsheetView(_ spreadsheetView: SpreadsheetView, cellForItemAt indexPath: IndexPath) -> Cell? {
        if indexPath.row == 0 {
            ...
            return cell
        }else if indexPath.section == 0 || indexPath.section == 1 || indexPath.section == 2 || indexPath.section == 3 {
            let cell = spreadsheetView.dequeueReusableCell(withReuseIdentifier: "CellTextXib", for: indexPath) as! CellTextXib
            cell.backgroundColor = .white
            cell.lblText.textColor = .black
            cell.gridlines.top = .solid(width: 1, color: UIColor.black.withAlphaComponent(0.175))
            ...
            if indexPath.section == 0 {
                text = itemArray[indexPath.row-1].budgetCategory
            }else if indexPath.section == 1 {
                text = convertDateString(itemArray[indexPath.row-1].fromDate)
            }else if indexPath.section == 2 {
                text = convertDateString(itemArray[indexPath.row-1].toDate)
            }else if indexPath.section == 3 {
                text = String(format: "£%.2f", itemArray[indexPath.row-1].cost ?? 0.0)
            }
            ...
            cell.btnAction.addAction { [weak self] in
                DispatchQueue.main.async { [weak self] in
                    ...
                    let api = ApiService.deleteEnegySubCoste(costId: itemArray[indexPath.row-1].costId ?? 0)
